...
# =============================================================
# Session 4: every /v1 request must carry an access token.
# Tokens are signed with the test-only key written by
# tests/helpers/jwks-server.js.
#
# Usage:
#   1. node tests/helpers/jwks-server.js &
#   2. start the service with OIDC_* pointing at http://127.0.0.1:9999
#   3. BASE_URL=http://localhost:3000 bash tests/contract/contract.test.sh
...
TOKEN_PEMOHON=$(node "$ROOT/tests/helpers/token.js" pemohon-a requests:read requests:write)
TOKEN_PETUGAS=$(node "$ROOT/tests/helpers/token.js" petugas-a requests:read distributions:read handovers:write)
TOKEN_GUDANG=$(node "$ROOT/tests/helpers/token.js" gudang-a packages:read)
...
echo ""
echo "=== Contract Conformance Tests (Session 4) ==="
echo "Base URL: $BASE"
echo ""
...
check "GET collection without token → 401" 401 "$STATUS"

STATUS=$(curl -s -o /dev/null -w "%{http_code}" \
  -H "Authorization: Bearer not-a-token" "$BASE/v1/assistance-requests")
check "GET collection with invalid token → 401" 401 "$STATUS"

# --- GET /v1/assistance-requests (collection) ---
echo ""
echo "[ GET /v1/assistance-requests ]"
STATUS=$(curl -s -o /dev/null -w "%{http_code}" \
  -H "Authorization: Bearer $TOKEN_PEMOHON" "$BASE/v1/assistance-requests")
check "GET collection → 200" 200 "$STATUS"

STATUS=$(curl -s -o /dev/null -w "%{http_code}" \
  -H "Authorization: Bearer $TOKEN_PEMOHON" "$BASE/v1/assistance-requests?status=approved")
check "GET collection with valid status filter → 200" 200 "$STATUS"

STATUS=$(curl -s -o /dev/null -w "%{http_code}" \
  -H "Authorization: Bearer $TOKEN_PEMOHON" "$BASE/v1/assistance-requests?urgency=high")
check "GET collection with valid urgency filter → 200" 200 "$STATUS"

STATUS=$(curl -s -o /dev/null -w "%{http_code}" \
  -H "Authorization: Bearer $TOKEN_PEMOHON" "$BASE/v1/assistance-requests?status=INVALID_STATUS")
check "GET collection with invalid status → 400" 400 "$STATUS"

STATUS=$(curl -s -o /dev/null -w "%{http_code}" \
  -H "Authorization: Bearer $TOKEN_PEMOHON" "$BASE/v1/assistance-requests?limit=0")
check "GET collection with limit=0 (below minimum) → 400" 400 "$STATUS"

STATUS=$(curl -s -o /dev/null -w "%{http_code}" \
  -H "Authorization: Bearer $TOKEN_PEMOHON" "$BASE/v1/assistance-requests?limit=101")
...
STATUS=$(curl -s -o /dev/null -w "%{http_code}" \
  -H "Authorization: Bearer $TOKEN_PEMOHON" "$BASE/v1/assistance-requests/req_7Kq91Ab")
check "GET single entity (seed data, owned by pemohon-a) → 200" 200 "$STATUS"

STATUS=$(curl -s -o /dev/null -w "%{http_code}" \
  -H "Authorization: Bearer $TOKEN_PEMOHON" "$BASE/v1/assistance-requests/req_ZZZZZZZ")
check "GET single entity (not found) → 404" 404 "$STATUS"

STATUS=$(curl -s -o /dev/null -w "%{http_code}" \
  -H "Authorization: Bearer $TOKEN_PEMOHON" "$BASE/v1/assistance-requests/invalid-format")
...
STATUS=$(curl -s -o /dev/null -w "%{http_code}" \
  -H "Authorization: Bearer $TOKEN_GUDANG" "$BASE/v1/packages")
check "GET packages collection → 200" 200 "$STATUS"

STATUS=$(curl -s -o /dev/null -w "%{http_code}" \
  -H "Authorization: Bearer $TOKEN_PEMOHON" "$BASE/v1/packages")
check "GET packages without packages:read scope → 403" 403 "$STATUS"

# --- GET /v1/distributions ---
echo ""
echo "[ GET /v1/distributions ]"
STATUS=$(curl -s -o /dev/null -w "%{http_code}" \
  -H "Authorization: Bearer $TOKEN_PETUGAS" "$BASE/v1/distributions")
check "GET distributions collection → 200" 200 "$STATUS"

STATUS=$(curl -s -o /dev/null -w "%{http_code}" \
  -H "Authorization: Bearer $TOKEN_PETUGAS" "$BASE/v1/distributions/dst_88bAa99")
check "GET single distribution (owned by petugas-a) → 200" 200 "$STATUS"

STATUS=$(curl -s -o /dev/null -w "%{http_code}" \
  -H "Authorization: Bearer $TOKEN_PETUGAS" "$BASE/v1/distributions/dst_ZZZZZZZ")
check "GET single distribution (not found) → 404" 404 "$STATUS"

# --- POST /v1/assistance-requests ---
echo ""
echo "[ POST /v1/assistance-requests ]"
KEY=$(node -e "console.log(require('crypto').randomUUID())" 2>/dev/null || python3 -c "import uuid; print(uuid.uuid4())" 2>/dev/null || echo "00000000-0000-4000-8000-000000000001")
...
STATUS=$(curl -s -o /dev/null -w "%{http_code}" \
  ...
  -d '{"distributionId":"dst_NOTEXIST","recipientNationalId":"9999999999999999","handedOverAt":"2026-09-19T10:00:00+07:00","fieldOfficerId":"ofc_TEST001"}')
...
check "POST handover with valid seed distribution (petugas-a) → 201" 201 "$STATUS"
